...
exports.getTree = async function (depth = 3) {
  const all = await Category.find({ is_active: true })
    .sort({ level: 1, name: 1 })
    .lean();
  const byParent = all.reduce(
    (m, c) => ((m[c.parent_id || "root"] ||= []).push(c), m),
    {}
  );
...
exports.update = async function (id, patch) {
  ...
  if (
    String(before.parent_id || "") !== String(doc.parent_id || "") ||
...
exports.remove = async function (id) {
  ...
  if (prodCount > 0)
    throw new Error("Không thể xoá: vẫn còn sản phẩm thuộc danh mục");
  await Category.findByIdAndDelete(id);
  return { ok: true };
};

exports.getAll = async function () {
  return Category.find({ is_active: true }).sort({ name: 1 }).lean();
};

exports.getCategoryBySlug = async function (slug) {
  return Category.findOne({ slug }).select("name slug").lean();
};
