...
(async () => {
  ...
  for (const r of roots) {
    const root = await ensureCategory({ name: r.name, gender_hint: r.gender_hint });

    const ao   = await ensureCategory({ name:"Áo",   parent_id: root._id, gender_hint:r.gender_hint });
    const quan = await ensureCategory({ name:"Quần", parent_id: root._id, gender_hint:r.gender_hint });
    await Promise.all([
      ensureCategory({ name:"Áo polo", parent_id: ao._id,   gender_hint:r.gender_hint }),
      ensureCategory({ name:"Áo thun", parent_id: ao._id,   gender_hint:r.gender_hint }),
      ensureCategory({ name:"Áo sơ mi",parent_id: ao._id,   gender_hint:r.gender_hint }),
      ensureCategory({ name:"Quần jean",  parent_id: quan._id, gender_hint:r.gender_hint }),
      ensureCategory({ name:"Quần short", parent_id: quan._id, gender_hint:r.gender_hint }),
    ]);
  }
